...
export default async function ClientPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: profile }, { data: client }, { data: contacts }, { data: notes }, { data: allClients }] = await Promise.all([
    ...
    sb.from('clients').select('*').eq('id', params.id).eq('owner_id', user.id).single(),
    sb.from('contacts').select('*').eq('client_id', params.id).order('is_primary', { ascending: false }),
    sb.from('notes').select('*').eq('client_id', params.id).order('pinned', { ascending: false }).order('created_at', { ascending: false }),
